...
export class GridComponent implements OnInit {
  ...
  private indexPosition: Array<[Position,number]> = new Array();
  private currentPosition: Position=new Position(0,0);
  private width : number = 10; 
  // Mock list that we recieve from crossword generator
  private mot:string[]=["aaaaaa", "*", "aaa","sadasd", "*", "asd",
                        "aaaaaa", "*", "aaa","sadasd", "*", "asd", 
                        "aaaaaa", "*", "aaa","sadasd", "*", "asd", 
                        "aaaaaa", "*", "aaa","sadasd", "*", "asd", 
                        "aaaaaa", "*", "aaa","sadasd", "*", "asd",];
  ...
  createIndex(){  //create the array that contain every position with its index number
    let j:number=1;
    for(let i : number = 0; i < this.wordChar.length; ++i){
      if(this.containIndex(i)){
        this.indexPosition.push([new Position(i%10,Math.floor(i/this.width)),j])
      }
    }
  }
